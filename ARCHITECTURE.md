# 🏛️ CityPulse & GarbageMaps — Technical Architecture & System Design

> For the comprehensive full-tier systems design specification, see [SYSTEM_ARCHITECTURE.md](file:///c:/Users/Shaban%20Chaudhary/Desktop/Projects/community%20projects/Garbage%20and%20crowd%20Management/garbagemap/SYSTEM_ARCHITECTURE.md).
...
- **Route Modules**:
  ...
  | `/api/admin-pass` | `routes/adminpass.js` | Secondary administrative passcode authorization |
  | `/api/heatmap` | `routes/heatmap.js` | Geospatial coordinates and severity-weighted points |
  | `/api/areas` | `routes/areas.js` | Area cleanliness scores and zone status |
  | `/api/analytics` | `routes/analytics.js` | Aggregated civic metrics and KPIs |
  ...
  | `/api/ai` | `routes/ai.js` | HuggingFace computer vision garbage classification |
  | `/api/wards` | `routes/wards.js` | Ward GeoJSON GIS boundaries, officer contacts, PDF/Excel reports |
  | `/api/store` | `routes/store.js` | Virtual economy, EcoCoins, cosmetic wardrobe, daily streaks |
  | `/api/comments` | `routes/comments.js` | Public report discussion threads with official update badges |
...
$$\text{Cleanliness Score} = \max\left(0, 100 - (\text{Count} \times 2) - (\text{High Severity} \times 5) - (\text{Medium Severity} \times 2)\right)$$
...
1. **Authentication Boundary**: Asymmetric GoTrue JWTs validated per-request via bearer headers.
2. **Role-Based Access Control (RBAC)**: Enforced via `users.role` checks and the secondary `ADMIN_PASS` gate.
3. **Database Row Level Security (RLS)**: Active on all tables in Supabase PostgreSQL.
4. **CORS Hardening**: Strict origin whitelisting allowing only verified domains.
