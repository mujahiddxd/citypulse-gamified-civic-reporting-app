-- ============================================
-- GarbageMaps - Complete Supabase SQL Schema
...
CREATE TABLE public.badges (
  ...
  icon TEXT,
  condition_type TEXT, -- 'xp', 'approved_count', 'type_specific'
...
-- ============================================
-- AREA SCORES TABLE (computed/cached)
-- ============================================
...
CREATE POLICY "Users can update own complaints" ON public.complaints FOR UPDATE USING (auth.uid() = user_id OR EXISTS(SELECT 1 FROM public.users WHERE id = auth.uid() AND role = 'admin'));
...
CREATE POLICY "User badges insertable by system" ON public.user_badges FOR INSERT WITH CHECK (true);

-- XP logs policies
CREATE POLICY "Users can view own XP logs" ON public.xp_logs FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "XP logs insertable by system" ON public.xp_logs FOR INSERT WITH CHECK (true);

-- Feedback policies
CREATE POLICY "Anyone can submit feedback" ON public.feedback FOR INSERT WITH CHECK (true);
CREATE POLICY "Admins can view all feedback" ON public.feedback FOR SELECT USING (EXISTS(SELECT 1 FROM public.users WHERE id = auth.uid() AND role = 'admin'));
...
-- ============================================
-- FUNCTION: Award XP and update level/badges
-- ============================================
